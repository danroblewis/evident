"""
Shared utilities for Evident conformance tests (port edition).
...
The binary under test is selected by the EVIDENT_CMD environment variable.
Set it to the path of the evident binary you are porting:

    EVIDENT_CMD=/path/to/your-evident pytest runtime-port/conformance/

If EVIDENT_CMD is unset the suite refuses to run — there is no built-in
default binary path inside this folder, because the whole point of the
runtime-port directory is that the binary lives elsewhere.
"""
...
# runtime-port/conformance/conftest.py → runtime-port/
PROJECT_ROOT = Path(__file__).parent.parent

_env_cmd = os.environ.get('EVIDENT_CMD')
if not _env_cmd:
    raise SystemExit(
        "EVIDENT_CMD is not set. Set it to the path of the evident "
        "binary you want to test, e.g. "
        "EVIDENT_CMD=./build/evident pytest runtime-port/conformance/"
    )
EVIDENT_CMD = shlex.split(_env_cmd)
...
def query(source: str, schema: str, given: dict[str, Any] | None = None,
    ...
    The evident CLI uses two different JSON formats:
      SAT:   exit 0, stdout = {binding: value, ...}  (just the bindings)
             OR        {"satisfied": true, "bindings": {...}}
      UNSAT: exit 1, stdout = {"satisfied": false}
    Both SAT shapes are accepted so the same suite runs against either
    style of port.
    """
