...
#
# build-compiler-smt2.sh — the one-time bootstrap → self-hosted handoff.
#
# Compiles the self-hosted compiler (compiler/compiler.ev) to a single
# runnable SMT-LIB program (compiler.smt2 at the repo root), using the
# bootstrap Rust runtime ONE LAST TIME. After this file exists, the
# kernel runs `compiler.smt2` to compile every other `.ev` file and the
...
#   1. scripts/flatten-evident.sh compiler/compiler.ev  > /tmp/…-flat.ev
#      (resolve every `import "…"` into one flat translation unit, since
#      kernel + compiler.smt2 does NOT do import resolution).
...
#      never leaves a half-written compiler.smt2).
#
# Usage:
#   scripts/build-compiler-smt2.sh              # build → compiler.smt2
#   scripts/build-compiler-smt2.sh --check-only # build → /tmp preview only
#
# Exit codes:
#   0  success
#   1  compiler/compiler.ev missing, flatten failed, or bad output
...
SRC="$ROOT/compiler/compiler.ev"
OUT="$ROOT/compiler.smt2"

CHECK_ONLY=0
case "${1:-}" in
    --check-only) CHECK_ONLY=1 ;;
    "") ;;
    *) echo "usage: build-compiler-smt2.sh [--check-only]" >&2; exit 1 ;;
esac

die() { echo "build-compiler-smt2: $*" >&2; exit 1; }
...
FLAT="$(mktemp -t compiler-flat.XXXXXX.ev)"
trap 'rm -f "$FLAT"' EXIT
...
if ! "$EVIDENT" emit "$FLAT" main -o "$STAGE"; then
    rc=$?
    rm -f "$STAGE"
    echo "build-compiler-smt2: bootstrap emit failed (exit $rc)" >&2
    exit "$rc"
fi
...
if [ "$CHECK_ONLY" -eq 1 ]; then
    ...
    echo "  compiler.smt2 is ready — kernel can now compile .ev files with it."
fi
