...
def cmd_execute(args):
    ...
    plugins = default_plugins(
        ...
        tcp_host   = getattr(args, 'host',   '127.0.0.1'),
        tcp_port   = getattr(args, 'port',   8080),
    )
    try:
...
def main():
    ...
    ex.add_argument('--host',   default='127.0.0.1',          help='TCP listen host (if TCPSocket is active)')
    ex.add_argument('--port',   type=int, default=8080,       help='TCP listen port (if TCPSocket is active)')
