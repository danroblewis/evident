...
| `seq_chains` (`extract_seq_effect_chains`) | **Evident-only** (`portable/seq_chains.rs::EvidentSeqChains`) — Rust walk + `node_name` DELETED (session PORT-seqchains) | `stdlib/passes/seq_chains.ev` (`seq_chains_walk` stack-FSM) | **full, sole impl** | Cut over: the canonical Rust `effect_loop/seq_chains.rs` walk is gone; the scheduler's Mode-2 dispatch-ordering path (`collect.rs`) derives `Seq(Effect)` ordering chains through `portable::seq_chains::extract_seq_effect_chains` (per-thread cached engine, WW resolver, bootstrap guard). This pass's recursion is a FLAT scan over the claim's TOP-LEVEL body items (the original never descends into sub-`Expr`s/subclaims), so the #19a cons-list stack substrate IS the `BodyItemList` itself — pop the head, emit-or-skip, recurse on the tail. The FSM (fed by the SHARED marshaler, UU) collects each recognized `Constraint(Eq(_, SeqLit))`'s RAW element `Expr`s; the Rust shim's `node_name` does the leaf string-keying — set-membership (in-solve string-eq blow-up, #18's cousin) AND synthetic-name `format!` (`hat_effs[0]`, `plat_effs[0].effs[0]`, needs int→string, a gap) — plus the all-elements-resolve gate. **Per-tick cost: strictly cheaper than before.** The walk is body-only (static), so raw chains are cached by body identity (`as_ptr`+`len`) and the Evident walk runs once per claim; per-tick is then an O(1) cache hit + the `node_name` resolution that already lived in Rust — vs. the old code re-walking the body in Rust every tick. Pinned chains (all `node_name` shapes + the all-resolve gate + body order) in `runtime/tests/seq_chains_correctness.rs`. |
...
| `introspect` (schema mutation) | **Evident-only** (`portable/introspect.rs`) — Rust `body.insert(0,…)` / `body[idx]=…` splice DELETED (session PORT-introspect) | `stdlib/passes/introspect.ev` (`introspect_replace` + `introspect_prepend` stack-FSMs) | **full, sole impl** | **The first AST-*rebuild* port**: the FSM takes a whole `SchemaDecl` + a mutation request and RETURNS the rebuilt `SchemaDecl` (every prior port returned a value-list; SEED-marshal made the seed marshaler round-trip `MatchPattern`/`BISubclaim` so a composite SchemaDecl can flow in via the seed and out via `run()`). `replace_body_item_in_claim` / `add_membership_to_claim` route their body rebuild through `portable::introspect::{replace_body_item, prepend_membership}` (per-thread cached engine, WW resolver). `introspect_replace` destructures the schema, walks the cons-list body to the target index with a reversed-prefix descend-then-rebuild, splices, and reconstructs `MakeSchemaDecl(…)`; `introspect_prepend` cons-es a membership at the head. **Load-time only** (sole caller is the passthrough-desugar auto-apply in `commands/common.rs`) — not on the per-tick/translate/scheduler path, so no bootstrap risk; `cached_runner`, not `guarded_runner`. Same "Evident owns the recursion, Rust owns the leaf" split: the bounds check (`idx < len`, an Int leaf) + the idempotency check (`var already declared?`, a string-set leaf dodging the in-solve string blow-up, #18's cousin) + the dual-update bookkeeping (mirror into `program.schemas`, flush the cache) stay in Rust. **Finding (the SESSION audit point):** the `*_to_value` encoder emits lists as cons-list `Value::Enum` spines (so the stack-FSM can pop a head) but the shared `decode_*` family reads the `Value::SeqEnum` shape Z3 extracts from `stdlib/ast.ev`'s `Seq(T)` fields — they are NOT inverses, so the shim normalizes the FSM's cons-list output to `SeqEnum`/`SeqStr` (`cons_to_seq`) before `decode_schema_decl`. And the marshaler's `SchemaDecl` is 4-field (drops `type_params`/`external`); the shim copies only the rebuilt `.body` back, preserving those — a whole-schema replace would lose them (parsed subclaims always carry the defaults, so body items still round-trip byte-identically). Pinned in `runtime/tests/introspect_correctness.rs` (synthetic battery + corpus vs the deleted Rust splice + the live dual-update path). Net LOC ≈ flat (proof, not Rust shrink — the small-pure-pass ceiling). |
| `toposort` (effect-dispatch ordering) | **Evident-only** (`portable/toposort.rs::EvidentToposort`) — Rust `topo_sort_with_random_tiebreak` Kahn's algorithm + the `EVIDENT_TOPOSORT_IMPL` env gate DELETED (session PORT-toposort) | `stdlib/toposort.ev` (`ToposortRanks` claim) | **full, sole impl** | Cut over: the dispatcher orders a tick's Effects solely through the Evident toposort, on a **dedicated** runtime (`portable::toposort::toposort`, per-thread cached engine, WW resolver). Unlike the stack-FSM passes this is a **direct constraint solve**, not a walk. The leaf split: Rust maps the dispatcher's node *names* → contiguous `0..n-1` integers and queries `ToposortRanks(n, edges, pos)`, whose edges index a rank array DIRECTLY (`pos[e.from] < pos[e.to]` — O(1)/edge); Rust then inverts `pos` back to a node-name ordering. The integer encoding is what makes it viable — the domain-typed generic `Toposort<T>` uses `position_of` (depth-n chained ITE, ×2/edge → O(n·#edges)) and took **13–42s** on a Mario frame (70 nodes/96 edges); `ToposortRanks` solves it in **~19ms** (no string theory, no ITE chains). **Setup-only**: orderings are memoized by `(nodes, edges)` shape (`DISPATCH_ORDER_CACHE`), so a shape-stable program (Mario emits the same graph every frame) pays one ~51ms tick-0 solve and per-tick steady state is a HashMap lookup — unchanged. One leaf stays Rust: **cycle recovery** (a cyclic graph is UNSAT → `None` → dispatch in input order + warn, keeping the program running — a policy decision, not the algorithm). Orderings pinned in `runtime/tests/toposort_correctness.rs`. |
...
| `seq_chains.ev` | **KEEP** | Sole impl of the scheduler's Mode-2 `Seq(Effect)` ordering-chain extraction (Rust `extract_seq_effect_chains` walk deleted in session PORT-seqchains). Backs `portable/seq_chains.rs`; per-tick on the dispatch path, so the body-only walk is cached by body identity. |
