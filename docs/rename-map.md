...
## merge additions (element-form/keyed work, named at merge time)

Variables introduced on main after the rename branch forked; named here
to match the conventions above.

| old | new | meaning |
| --- | --- | ------- |
| `uev_acc_nm` | `variant_acc_name` | name of the variant whose accessor lands next tick |
| `uev_acc_on` | `variant_acc_on` | an accessor is pending (the Bool gate) |
| `sv_cur_nm` | `setvar_cur_name` | set-literal walk: active set, by name |
| `c_stv_pfx_hit` | `setvar_pfx_hit` | line name (+scope prefix) matches a set var |
| `c_stv_bare_hit` | `setvar_bare_hit` | bare line name matches a set var |
| `c_stv_found` | `setvar_found` | either match hit |
| `c_stv_nm` | `setvar_name` | the matched set-var name |
| `c_stv_kidx` | `setvar_slot` | element-type slot of the matched set |
| `c_smem_pfx_hit` | `setmem_pfx_hit` | membership type (+prefix) matches a set var |
| `c_smem_bare_hit` | `setmem_bare_hit` | bare membership type matches a set var |
| `d_sl_bare_hit` | `len_set_bare_hit` | str_len arg matches a set var bare |
| `d_sl_pfx_hit` | `len_set_pfx_hit` | str_len arg matches with scope prefix |
| `d_sl_set` | `len_of_setvar` | str_len arg names a set var |
| `d_sl_nm` | `len_set_name` | the matched set-var name |
| `mp_t1_uev` | `fold_tester1_user` | arm-1 tester from the user-variant registry |
| `mp_t2_uev` | `fold_tester2_user` | arm-2 tester from the user-variant registry |
| `mp_acc1_uev` | `fold_acc1_user` | arm-1 accessor from the registry |
| `mp_acc2_uev` | `fold_acc2_user` | arm-2 accessor from the registry |
| `mp_dacc_uev` | `fold_def_acc_user` | default-arm accessor from the registry |
| `d_m_t_uev` | `matches_tester_user` | matches-tester from the registry |
| `d_lk_evt` | `lookup_enum_val` | symbol value from the enum-values registry |
