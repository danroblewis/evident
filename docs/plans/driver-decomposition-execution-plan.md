...
| gd | (this run) | `driver_guard.ev` (`fsm DriverGuard`) — the G1 conditional-inline (`cond ⇒ ClaimName`) guarded-splice phase (guard recognizer + claim-body fetch + arm/disarm against the inline-frame stack) | 69 (33 body) | ~14 bus IN (>8, JUSTIFIED §6.4: a guarded jump couples parse-result + claim index + inline-frame stack). Body 33 < 40 soft floor but NOT a trivial hoist: it owns the il_guard/il_gd guard-depth carry + the entire `cond ⇒ ClaimName` corpus feature — a complete distinct stateful phase. | arm_guard (jump-fire gate + d_h_top 99 ⇒ il_guard latches 99) | EQUIV; manifest unchanged | 1624 |
| lm | (this run) | `driver_litmem.ev` (`fsm DriverLitMem`) — the pmode-7 set-literal (`x ∈ {…}`/`∉`, or-folded membership) + pmode-8 Seq(Int)-literal (one registered seq var) membership walks, paired (both lower a literal-collection membership line) | 90 (37 body) | ~10 bus IN (slightly >8, JUSTIFIED §6.4: two literal-collection shapes share the parse gate + classifier name/sort views; splitting → two <40 stubs duplicating the read surface, the §3 anti-pattern) | seq_register (latch-once: d_seq_mem with changing name ⇒ sq_name stays "myseq"); set_entry (∉ entry ⇒ ms_name "x" ∧ ms_neg true) | EQUIV; manifest unchanged | 1572 |
