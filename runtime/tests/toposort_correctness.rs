...
use evident_runtime::portable::toposort::toposort;
...
fn linear_chain_forces_unique_order() {
    ...
    let out = toposort(&nodes(4), &edges).expect("acyclic → Some");
...
fn eight_node_dag_respects_every_edge() {
    ...
    let out = toposort(&nodes(8), &edges).expect("acyclic → Some");
    let mut sorted = out.clone();
    sorted.sort();
...
fn empty_edges_returns_all_nodes() {
    let out = toposort(&nodes(5), &[]).expect("no edges → Some");
    let mut sorted = out.clone();
    sorted.sort();
...
fn cycle_is_none() {
    ...
    assert_eq!(toposort(&nodes(3), &edges), None, "cyclic graph → None");
}

#[test]
fn empty_graph_is_empty() {
    assert_eq!(toposort(&[], &[]), Some(Vec::new()));
}

#[test]
fn production_entry_forced_order_and_cycle() {
    // `portable::toposort::toposort` is what `effect_loop::collect` calls.
    // The cached per-thread runner gives the forced order on a chain and
    // recovers a cycle as None.
