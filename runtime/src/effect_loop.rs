...
use crate::ast::{Effect, EffectResult, BodyItem, Expr, BinOp};
...
pub(crate) fn collect_dispatchable_effects(
    ...
    let _ = rt;        // reserved for future Evident-side toposort
    let _ = claim_name;
    let _ = bindings;  // (used below; re-introduced via re-borrow)
    let mut nodes: Vec<String> = bindings.iter()
        .filter_map(|(name, v)| match v {
            Value::Enum { enum_name, .. } if enum_name == "Effect" => Some(name.clone()),
            _ => None,
        })
        .collect();
    ...
    // body Constraint of shape `seq_name = ⟨a, b, c⟩` (or the equality
    // generated by `seq_name ∈ Seq(Effect) = ⟨…⟩` sugar) contributes
    // edges (a, b), (b, c). Only Identifier elements that name actual
    // Effect bindings count — anonymous Effects inside a Seq aren't
    // dispatched in this mode.
    let node_set: HashSet<&String> = nodes.iter().collect();
    let edges = match rt.get_schema(claim_name) {
    ...
    if edges.is_empty() {
        return resolve_nodes_to_effects(&nodes, bindings);
    }

    let sorted_names = topo_sort_with_random_tiebreak(&nodes, &edges, &mut rng);
    resolve_nodes_to_effects(&sorted_names, bindings)
}
        ...
        if let BodyItem::Constraint(Expr::Binary(BinOp::Eq, lhs, rhs)) = item {
            ...
            let names: Vec<&String> = seq_items.iter()
                .filter_map(|e| match e {
                    Expr::Identifier(n) if effect_node_set.contains(n) => Some(n),
                    _ => None,
                })
                .collect();
            // Only emit edges from chains of fully-named Effect bindings.
            // A Seq with even one anonymous expression isn't a clean
            // ordering chain — bail (the user can fix by binding).
