...
**Last action:** Two unrelated fixes that landed together:
  ...
  - **Standalone binary from `cargo build --release`.** `libz3.dylib`
    ...
    `runtime/scripts/cc-wrapper.sh` wraps `cc`, runs the link,
  ...
  - **Bilingual conformance harness.** Modified
    ...
    against Rust: `EVIDENT_CMD="$PWD/runtime/target/release/evident"
...
`translate_seq_lit_eq` (in `runtime/src/translate.rs`) with a real
...
**Last action (slice 2 of v1.6):** Two small surface-area additions.
  ...
  1. `cmd_execute` in `runtime/src/main.rs` now accepts the same
  ...
  2. Bare claim names in a body are now resolved as names-match
     ...
     `runtime/src/translate.rs` at translate time (not in the
...
blocking-stdin step loop. New `runtime/src/executor.rs`:
...
- [x] **`execute` subcommand (headless v1).** New `runtime/src/executor.rs`
