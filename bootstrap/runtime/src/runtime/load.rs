//! Source loading: parse + run pre-translation passes + cache flush.
...
impl EvidentRuntime {
    /// Parse and load Evident source.
    pub fn load_source(&mut self, src: &str) -> Result<(), RuntimeError> {
        self.load_source_with_base(src, None)
    }

    /// Load source from a file; records the canonical path for import resolution
    /// and cycle detection.
    ...
    pub(super) fn load_source_with_base(&mut self, src: &str, base: Option<&Path>) -> Result<(), RuntimeError> {
        ...
        for s in &prog.schemas {
            ...
            super::register_subclaims(&s.body, &mut self.schemas);
        }
    ...
    pub(super) fn resolve_import(&self, import_path: &str, base: Option<&Path>) -> Result<PathBuf, RuntimeError> {
        ...
        Err(RuntimeError::Io(format!("import not found: {:?}", import_path)))
    }
}
