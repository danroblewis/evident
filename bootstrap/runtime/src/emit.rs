...
fn discover_state_fields(env: &HashMap<String, crate::core::Var<'static>>) -> Vec<(String, String)> {
    ...
    for (name, var) in env {
        ...
        // Dotted name = a record/sub-schema field const (`c.n` for `c ∈ Counter`).
        // Records are a compile-time view; the carried state is the primitive
        // scalar components. The kernel carries a dotted-named primitive field
        // exactly like any other primitive (it reads `<rec>.<field>` from the
        // model and re-asserts `_<rec>.<field>` next tick — both are valid SMT
        // consts). We opt a field into carry ONLY when the program declared the
        // matching prev-tick record (`_c ∈ Counter` ⇒ env has `_c.n`); this is
        // the user's explicit carry intent and keeps non-carried records (and
        // every program that never mentions `_<rec>`) byte-identical to before.
        if name.contains('.') && !env.contains_key(&format!("_{name}")) { continue; }
        let ty = match var {
            Var::IntVar(_)     => "Int".to_string(),
            Var::BoolVar(_)    => "Bool".to_string(),
            Var::RealVar(_)    => "Real".to_string(),
            Var::StrVar(_)     => "String".to_string(),
            // PinnedInt is a compile-time constant: it is folded to a
            // literal in the body and never declared as a Z3 const, so
            // the kernel cannot read it from the model. It is also
            // re-derived from source every tick, so it never needs to
            // carry. Excluding it keeps the manifest in sync with the
            // declarations actually emitted. (Including it produced a
            // state-field with no backing const → kernel exit 3.)
            Var::PinnedInt(_)  => continue,
