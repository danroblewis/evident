...
def report_md(path, title, transition, submodels, init, fuel, done=None):
    ...
    L = [f"# {title}", "",
         "Each sub-model is shown on its own (symbolic interface), then the "
         "**combined** model the runtime actually solves. The recursion is owned "
         "by the unroller, not Python's stack.", ""]
    for m in submodels:
    ...
    L += [f"## transition `{transition.name}`  (one step: state → state′){uses}", "",
          ...
          f"This is what the **one-shot** strategy solves ({one_vars} variables — "
          "grows with depth).", "", "```", pretty.goal(cons), "```", "",
          "## run result", "",
